...
export const calculateRonCondition = (
    ...
    if (isOya) {
        // 親の場合
        if (simulateRon(points, selfName, opponentName, homba, 1500, sharedPoint, continuePoint) === selfName)
            return '30符1飜(1500点)';
        if (simulateRon(points, selfName, opponentName, homba, 2000, sharedPoint, continuePoint) === selfName)
            return '40符1飜(2000点)';
        if (simulateRon(points, selfName, opponentName, homba, 2400, sharedPoint, continuePoint) === selfName)
            return '七対子2飜(2400点)';
        if (simulateRon(points, selfName, opponentName, homba, 2900, sharedPoint, continuePoint) === selfName)
            return '30符2飜(2900点)';
        if (simulateRon(points, selfName, opponentName, homba, 3900, sharedPoint, continuePoint) === selfName)
            return '40符2飜(3900点)';
        if (simulateRon(points, selfName, opponentName, homba, 4800, sharedPoint, continuePoint) === selfName)
            return '七対子3飜(4800点)';
        if (simulateRon(points, selfName, opponentName, homba, 5800, sharedPoint, continuePoint) === selfName)
            return '30符3飜(5800点)';
        if (simulateRon(points, selfName, opponentName, homba, 7700, sharedPoint, continuePoint) === selfName)
            return '40符3飜(7700点)';
        if (simulateRon(points, selfName, opponentName, homba, 9600, sharedPoint, continuePoint) === selfName)
            return '七対子4飜(9600点)';
        if (simulateRon(points, selfName, opponentName, homba, 12000, sharedPoint, continuePoint) === selfName)
            return '満貫(12000点)';
        if (simulateRon(points, selfName, opponentName, homba, 18000, sharedPoint, continuePoint) === selfName)
            return '跳満(18000点)';
        if (simulateRon(points, selfName, opponentName, homba, 24000, sharedPoint, continuePoint) === selfName)
            return '倍満(24000点)';
        if (simulateRon(points, selfName, opponentName, homba, 36000, sharedPoint, continuePoint) === selfName)
            return '三倍満(36000点)';
        if (simulateRon(points, selfName, opponentName, homba, 48000, sharedPoint, continuePoint) === selfName)
            return '役満(48000点)';
        return '目無し';
    } else {
        // 子の場合
        if (simulateRon(points, selfName, opponentName, homba, 1000, sharedPoint, continuePoint) === selfName)
            return '30符1飜(1000点)';
        if (simulateRon(points, selfName, opponentName, homba, 1300, sharedPoint, continuePoint) === selfName)
            return '40符1飜(1300点)';
        if (simulateRon(points, selfName, opponentName, homba, 1600, sharedPoint, continuePoint) === selfName)
            return '七対子2飜(1600点)';
        if (simulateRon(points, selfName, opponentName, homba, 2000, sharedPoint, continuePoint) === selfName)
            return '30符2飜(2000点)';
        if (simulateRon(points, selfName, opponentName, homba, 2600, sharedPoint, continuePoint) === selfName)
            return '40符2飜(2600点)';
        if (simulateRon(points, selfName, opponentName, homba, 3200, sharedPoint, continuePoint) === selfName)
            return '七対子3飜(3200点)';
        if (simulateRon(points, selfName, opponentName, homba, 3900, sharedPoint, continuePoint) === selfName)
            return '30符3飜(3900点)';
        if (simulateRon(points, selfName, opponentName, homba, 5200, sharedPoint, continuePoint) === selfName)
            return '40符3飜(5200点)';
        if (simulateRon(points, selfName, opponentName, homba, 6400, sharedPoint, continuePoint) === selfName)
            return '七対子4飜(6400点)';
        if (simulateRon(points, selfName, opponentName, homba, 8000, sharedPoint, continuePoint) === selfName)
            return '満貫(8000点)';
        if (simulateRon(points, selfName, opponentName, homba, 12000, sharedPoint, continuePoint) === selfName)
            return '跳満(12000点)';
        if (simulateRon(points, selfName, opponentName, homba, 16000, sharedPoint, continuePoint) === selfName)
            return '倍満(16000点)';
        if (simulateRon(points, selfName, opponentName, homba, 24000, sharedPoint, continuePoint) === selfName)
            return '三倍満(24000点)';
        if (simulateRon(points, selfName, opponentName, homba, 32000, sharedPoint, continuePoint) === selfName)
            return '役満(32000点)';
        return '目無し';
    }
};
